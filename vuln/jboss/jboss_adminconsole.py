...
class VulnChecker(VulnCheck):
    def __init__(self, ip_and_port_list):
        self._name = 'jboss_adminconsole'
        self.info = "Check if the JBoss path /admin-console/ exists for CVE-2010-1871"
        self.keyword = ['all', 'jboss', 'rce', 'cve_2010_1871', 'web', 'intranet', 'danger', '8080']
        self.default_ports_list = WEB_PORTS_LIST
        VulnCheck.__init__(self, ip_and_port_list)

    def _check(self, ip, port):
        result = "Maybe jBoss vuln CVE-2010-1871"
        url1 = "http://%s:%d/admin-console/"%(ip, int(port)) if add_web_path == "" else "http://%s:%d/%s/admin-console/"%(ip, int(port), add_web_path)
        url2 = "https://%s:%d/admin-console/"%(ip, int(port)) if add_web_path == "" else "https://%s:%d/%s/admin-console/"%(ip, int(port), add_web_path)
        try:
