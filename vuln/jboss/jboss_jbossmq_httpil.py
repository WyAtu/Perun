...
class VulnChecker(VulnCheck):
    def __init__(self, ip_and_port_list):
        self._name = 'jboss_jbossmq_httpil'
        self.info = "Check if the JBoss path /jbossmq-httpil/HTTPServerILServlet exists for CVE-2017-7504"
        self.keyword = ['all', 'jboss', 'rce', 'cve_2017_7504', 'web', 'intranet', 'danger', '8080']
        self.default_ports_list = WEB_PORTS_LIST
        VulnCheck.__init__(self, ip_and_port_list)

    def _check(self, ip, port):
        result = "Maybe jBoss vuln CVE-2017-7504"
        url1 = "http://%s:%d/jbossmq-httpil/HTTPServerILServlet"%(ip, int(port)) if add_web_path == "" else "http://%s:%d/%s/jbossmq-httpil/HTTPServerILServlet"%(ip, int(port), add_web_path)
        url2 = "https://%s:%d/jbossmq-httpil/HTTPServerILServlet"%(ip, int(port)) if add_web_path == "" else "https://%s:%d/%s/jbossmq-httpil/HTTPServerILServlet"%(ip, int(port), add_web_path)
        try:
