...
class VulnChecker(VulnCheck):
    def __init__(self, ip_and_port_list):
        self._name = 'jboss_webconsole'
        self.info = "Check if the JBoss path /web-console/Invoker exists for CVE-2013-4810"
        self.keyword = ['all', 'jboss', 'rce', 'cve_2013_4810', 'web', 'intranet', 'danger', '8080']
        self.default_ports_list = WEB_PORTS_LIST
        VulnCheck.__init__(self, ip_and_port_list)

    def _check(self, ip, port):
        result = "Maybe jBoss vuln CVE-2013-4810"
        url1 = "http://%s:%d/web-console/Invoker"%(ip, int(port)) if add_web_path == "" else "http://%s:%d/%s/web-console/Invoker"%(ip, int(port), add_web_path)
        url2 = "https://%s:%d/web-console/Invoker"%(ip, int(port)) if add_web_path == "" else "https://%s:%d/%s/web-console/Invoker"%(ip, int(port), add_web_path)
        try:
